...
exports.handler = async function (event) {
  ...
  try {
    ...
    return {
      ...
      headers: CORS_HEADERS,
      body: JSON.stringify({
        error: 'Failed to save application. Payment successful — contact support with ref: ' + flwTxRef
      })
    };
  }
  ...
  try {
    ...
    if (!paymentInsertResponse.ok) {
      ...
      console.error('Payments insert failed:', paymentInsertResponse.status, paymentErrorText);
    } else {
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html:
          ...
          (supervisorName ? '<p><strong>Supervisor:</strong> ' + supervisorName + ' — ' + (supervisorEmail || 'no email') + '</p>' : '') +
          ...
          '<p style="color:#6B7280">' + toolsFamiliar + '</p>' +
          '<p><strong>Why TechNuVate:</strong></p>' +
          '<p style="color:#6B7280">' + whyTechnuvate + '</p>' +
          '<hr style="margin:16px 0;border:none;border-top:1px solid #E4E8F0">' +
