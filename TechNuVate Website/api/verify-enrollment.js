...
module.exports = async function handler(req, res) {
  ...
  if (!flwTxRef || !flwTxId) {
    res.setHeader('Access-Control-Allow-Origin', '*');
    return res.status(400).json({ error: 'flw_tx_ref and flw_tx_id are required.' });
  }
  ...
  if (!isTestMode) {
    ...
    try {
      ...
      res.setHeader('Access-Control-Allow-Origin', '*');
      return res.status(502).json({ error: 'Payment verification network error. Please contact support.' });
    }
    ...
    try {
      ...
      res.setHeader('Access-Control-Allow-Origin', '*');
      return res.status(502).json({ error: 'Payment verification response error.' });
    }
    ...
    if (
      ...
      res.setHeader('Access-Control-Allow-Origin', '*');
      return res.status(400).json({ error: 'Payment could not be verified. No records created.' });
    }
